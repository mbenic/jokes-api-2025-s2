<x-admin-layout>
    ...
    <section class="py-12 mx-12 space-y-4">
        ...
        <form class="flex flex-col gap-4"
              method="post"
              action="{{ route('admin.jokes.destroy', $joke) }}"
        >
            @csrf
            @method('delete')
            ...
            <div class="my-6 bg-gray-200 text-red-500 rounded-lg font-semibold flex flex-row overflow-hidden">
                ...
                <h3 class="pb-4 text-2xl">
                    {{ __('Warning! Deleting category') }} - {{ $joke->title }}
                </h3>

                <p class="text-xl">
                    {{ __('Are you sure you wish to delete the') }} "{{ $joke->title }}" {{ __('joke') }}?
                </p>
                </div>
            </div>

            <div class="flex flex-row justify-start">
                <x-link-primary-button href="{{ route('admin.jokes.index') }}"
                                         class="px-12 mr-6 ">
