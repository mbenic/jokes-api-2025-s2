    ...
    <section class="py-6 mx-12 space-y-4">
        ...
        <nav class="flex flex-row justify-between">
            <div class="flex gap-4 justify-end">
                ...
                <x-link-primary-button class="" href="{{ route('admin.jokes.create') }}">
                    <i class="fa-solid fa-plus pr-2"></i>
                    {{ __('Add Jokes') }}
                </x-link-primary-button>

                <x-link-secondary-button href="{{ route('admin.jokes.index') }}"
                                         class="bg-gray-900 text-black hover:bg-gray-500">
                    <i class="fa-solid fa-list pr-2"></i>
                    {{ __('All Jokes') }}
                </x-link-secondary-button>

                <form method="post"
                      action="{{ route('admin.jokes.trash.recover.all') }}"
                      class="">
                    @csrf
                ...
                <form method="post"
                      action="{{ route('admin.jokes.trash.remove.all') }}"
                      class="">
                    @csrf
                    @method('delete')
            ...
            <div class="flex gap-6">
                ...
                <form action="{{ route('admin.jokes.trash') }}" name="searchForm"
            ...
            @forelse($jokes as $joke)
                <tr class="hover:bg-gray-300 transition">
                    ...
                    <td class="p-2 flex gap-4">
                        ...
                        <form method="post"
                              action="{{ route('admin.jokes.trash.restore.one', $joke) }}">
                            @csrf
                        ...
                        <form method="post"
                              action="{{ route('admin.jokes.trash.remove.one', $joke) }}">
                            @csrf
